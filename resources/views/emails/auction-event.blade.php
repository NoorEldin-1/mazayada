...
<head>
    ...
    <title>{{ __("mail.events.{$event}.subject", $params) }}</title>
</head>
                ...
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e6eae8;">
                    ...
                    <tr>
                        <td style="padding:32px;text-align:{{ locale_dir() === 'rtl' ? 'right' : 'left' }};">
                            <p style="margin:0 0 10px;font-size:16px;color:#111827;font-weight:600;">{{ __('mail.events_common.greeting', ['name' => $notifiable->name ?? '']) }}</p>
                            <p style="margin:0 0 24px;font-size:14px;line-height:1.7;color:#4b5563;">{{ __("mail.events.{$event}.line", $params) }}</p>

                            @if($actionUrl)
                            <div style="text-align:center;margin:8px 0 0;">
                                <a href="{{ $actionUrl }}" style="display:inline-block;background:#15573f;color:#ffffff;text-decoration:none;border-radius:10px;padding:12px 28px;font-size:14px;font-weight:600;">{{ __('mail.events_common.cta') }}</a>
                            </div>
                            @endif
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td style="padding:18px 32px;background:#f9fafb;border-top:1px solid #eef1f0;text-align:center;">
                            <p style="margin:0;font-size:12px;color:#9ca3af;">{{ __('mail.events_common.footer') }}</p>
                        </td>
                    </tr>
                </table>
            </td>
