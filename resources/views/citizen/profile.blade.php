...
<div class="grid lg:grid-cols-2 gap-5">
    ...
    <x-ui.card :title="__('profile.identity_card')">
        <div class="text-center mb-5">
            <div class="w-20 h-20 rounded-[20px] text-white grid place-items-center font-bold text-[28px] mx-auto mb-3" style="background:linear-gradient(135deg,#2D6A4F,#1B4D3E)">{{ mb_substr(auth()->user()->first_name_ar, 0, 1) }}</div>
            <h3 class="m-0 mb-1 text-lg text-ink">{{ auth()->user()->fullNameAr() }}</h3>
            <p class="text-muted text-[13px] m-0">{{ auth()->user()->email }}</p>
        </div>
        <div class="grid gap-3">
            <div class="flex justify-between py-2.5 border-b border-line">
                <span class="text-muted text-sm">{{ __('profile.nin') }}</span>
                <span class="num text-[13px] font-semibold" dir="ltr">{{ auth()->user()->nin }}</span>
            </div>
            <div class="flex justify-between py-2.5 border-b border-line">
                <span class="text-muted text-sm">{{ __('profile.phone') }}</span>
                <span class="num text-[13px] font-semibold" dir="ltr">{{ auth()->user()->phone }}</span>
            </div>
            <div class="flex justify-between py-2.5 border-b border-line">
            ...
            <div class="flex justify-between py-2.5">
                ...
                <span class="num text-[13px]">{{ auth()->user()->created_at->format('Y-m-d') }}</span>
            </div>
        </div>
    </x-ui.card>
