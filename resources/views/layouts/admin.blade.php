...
<aside class="dash-side fixed inset-y-0 start-0 z-40 w-64 flex flex-col bg-[var(--sidebar-bg)] text-white border-e border-white/5">
    ...
    <nav class="flex-1 overflow-y-auto px-2.5 py-3.5 flex flex-col gap-1">
        ...
        @can('reports.view')
        <x-ui.nav-link tone="onPrimary" :href="route('admin.reports.index')" :active="request()->routeIs('admin.reports.*')">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg>
            <span>{{ __('admin.nav_reports') }}</span>
        </x-ui.nav-link>
        @endcan

        @can('auction-reports.viewAny')
        <x-ui.nav-link tone="onPrimary" :href="route('admin.auction-reports.index')" :active="request()->routeIs('admin.auction-reports.*')">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
            <span>{{ __('admin.nav_auction_reports') }}</span>
            @if(($auctionReportPendingCount ?? 0) > 0)
                <span class="ms-auto min-w-[20px] h-5 px-1.5 grid place-items-center rounded-full bg-danger text-white text-[11px] font-bold leading-none">{{ $auctionReportPendingCount }}</span>
            @endif
        </x-ui.nav-link>
        @endcan
