...
{{-- novalidate: this form is long enough that native validation — which stops at
     one field per submit with no overview — made it impossible to see what was
     missing. The server validates everything in one pass and the errors come
     back as the summary rendered by layouts.admin plus the inline @error
     messages below each field. --}}
<form method="POST" action="{{ route('admin.auctions.store') }}" enctype="multipart/form-data" novalidate>
    @csrf
    ...
    <x-ui.card :title="__('admin.auctions.sec_titles')" class="mb-6">
        ...
        <div class="field">
            ...
            {{-- Secondary-language fields stay optional, but the consequence of
                 leaving them blank is now stated instead of surprising the
                 visitor with Arabic text in the FR/EN interface. --}}
            <small style="color:var(--ink-muted)">{{ __('admin.auctions.translation_hint') }}</small>
    ...
    <x-ui.card :title="__('admin.auctions.sec_classification')" class="mb-6">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,200px),1fr));gap:1rem">
            ...
            <div class="field">
                ...
                <select id="wilaya_id" name="wilaya_id" class="select" required data-selected-commune="{{ old('commune_id') }}">
            ...
            <div class="field">
                ...
                <input type="text" id="mayor_name" name="mayor_name" class="input" value="{{ old('mayor_name') }}">
    ...
    @include('admin.auctions.partials.media-fields', ['auction' => null])
    ...
    <x-ui.card :title="__('admin.auctions.sec_pricing')" class="mb-6">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(100%,200px),1fr));gap:1rem">
            <div class="field">
                ...
                <input type="number" id="opening_price" name="opening_price" class="input num" value="{{ old('opening_price') }}"
                       min="0" max="{{ config('mazayada.limits.max_price_dzd') }}" step="0.01" required>
                <small style="color:var(--ink-muted)">{{ __('admin.auctions.price_max_hint', ['max' => number_format(config('mazayada.limits.max_price_dzd'))]) }}</small>
    ...
    {{-- Entity & responsible staff. Only a SUPER_ADMIN picks the owning entity;
         other staff are pinned to their own entity but may still tag a colleague. --}}
    <x-ui.card :title="__('admin.auctions.sec_entity')" class="mb-6">
        ...
        <div class="field">
            ...
            <select id="entity_id" name="entity_id" class="select" required data-selected-staff="{{ old('entity_user_id') }}">
        ...
        <div class="field">
            ...
            <select id="entity_user_id" name="entity_user_id" class="select">
                ...
                @foreach($entityUsers as $staff)
                    <option value="{{ $staff->id }}" {{ old('entity_user_id') == $staff->id ? 'selected' : '' }}>{{ $staff->full_name }}</option>
                @endforeach
            </select>
            @error('entity_user_id') <small class="text-danger text-xs mt-1">{{ $message }}</small> @enderror
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        //     and the post_max_size budget check (shared with the edit form) ---
