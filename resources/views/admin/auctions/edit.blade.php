...
@section('title', 'تعديل المزايدة')
@section('page-title', 'تعديل المزايدة')
...
<form method="POST" action="{{ route('admin.auctions.update', $auction) }}">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
            <div class="field">
                ...
                <select id="category_id" name="category_id" class="select" required>
                    ...
                    @foreach($categories as $category)
                        <option value="{{ $category->id }}" {{ old('category_id', $auction->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name_ar }}</option>
                    @endforeach
                </select>
            ...
            <div class="field">
                ...
                <select id="wilaya_id" name="wilaya_id" class="select" required>
                    ...
                    @foreach($wilayas as $wilaya)
                        <option value="{{ $wilaya->id }}" {{ old('wilaya_id', $auction->wilaya_id) == $wilaya->id ? 'selected' : '' }}>{{ $wilaya->code }} - {{ $wilaya->name_ar }}</option>
                    @endforeach
                </select>
            ...
            <div class="field">
                ...
                <select id="auction_type" name="auction_type" class="select" required>
                    ...
                    <option value="SALE" {{ old('auction_type', $auction->auction_type?->value) === 'SALE' ? 'selected' : '' }}>بيع</option>
                    <option value="LEASE" {{ old('auction_type', $auction->auction_type?->value) === 'LEASE' ? 'selected' : '' }}>إيجار</option>
                </select>
            ...
            <div class="field">
                ...
                <select id="condition" name="condition" class="select" required>
                    ...
                    <option value="NEW" {{ old('condition', $auction->condition?->value) === 'NEW' ? 'selected' : '' }}>جديد</option>
                    <option value="GOOD" {{ old('condition', $auction->condition?->value) === 'GOOD' ? 'selected' : '' }}>جيد</option>
                    <option value="FAIR" {{ old('condition', $auction->condition?->value) === 'FAIR' ? 'selected' : '' }}>مقبول</option>
                    <option value="POOR" {{ old('condition', $auction->condition?->value) === 'POOR' ? 'selected' : '' }}>سيئ</option>
                    <option value="SCRAP" {{ old('condition', $auction->condition?->value) === 'SCRAP' ? 'selected' : '' }}>خردة</option>
                </select>
        ...
        <div class="field">
            ...
            <select id="entity_id" name="entity_id" class="select" required>
                ...
                @foreach($entities as $entity)
                    <option value="{{ $entity->id }}" {{ old('entity_id', $auction->entity_id) == $entity->id ? 'selected' : '' }}>{{ $entity->name_ar }}</option>
                @endforeach
            </select>
    ...
    {{-- Submit --}}
    <button type="submit" class="btn btn-primary btn-block btn-lg">حفظ التعديلات</button>
</form>
