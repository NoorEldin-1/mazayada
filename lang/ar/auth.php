<?php

return [
    // ===== Auth layout — marketing side panel =====
    'left_badge' => 'المنصة الوطنية للمزايدات',
    'left_title' => 'منصة المزايدات الرقمية الأولى في الجزائر',
    'left_desc' => 'انضم إلى آلاف المستخدمين الذين يشاركون في المزايدات العمومية بكل شفافية وأمان عبر منصتنا الرقمية.',
    'testimonial' => 'منصة مزايدة غيّرت تجربتنا في المشاركة في المزايدات العمومية. عملية شفافة وسهلة الاستخدام.',
    'testimonial_name' => 'محمد بن عمر',
    'testimonial_role' => 'مدير شركة مقاولات',
    'stat_active_auctions' => 'مزايدة نشطة',
    'stat_registered_users' => 'مستخدم مسجل',
    'stat_wilayas' => 'ولاية',

    // ===== Marketing side panel — instructions carousel =====
    'carousel' => [
        'aria_label' => 'شرائح تعليمات وشروط المنصة',
        'prev' => 'الشريحة السابقة',
        'next' => 'الشريحة التالية',
        'go_to' => 'الانتقال إلى الشريحة :num',

        // Slide 1 — entry rules
        'input_title' => 'تعليمات الإدخال',
        'input_nin' => 'رقم التعريف الوطني (NIN): 18 رقمًا بالضبط.',
        'input_phone' => 'رقم الهاتف: 10 أرقام تبدأ بـ 05 أو 06 أو 07.',
        'input_age' => 'المشاركة متاحة لمن بلغ 18 سنة فأكثر.',

        // Slide 2 — KYC
        'kyc_title' => 'خطوات التحقق من الهوية',
        'kyc_body' => 'بعد التسجيل هناك خطوة تحقق إلزامية: رفع وثائق هويتك وصورة سيلفي. أكملها في وقتها لتفادي تعليق حسابك.',

        // Slide 3 — security & data protection
        'security_title' => 'الأمان وحماية البيانات',
        'security_body' => 'منصة حكومية آمنة تخضع لقانون حماية البيانات الجزائري (18-07). بياناتك تبقى داخل الجزائر ولا تُنقل إلى الخارج.',

        // Slide 4 — how to join auctions
        'auction_title' => 'آلية المشاركة في المزادات',
        'auction_body' => 'للدخول في أي مزاد تدفع رسوم دخول ومبلغ تأمين (قابل للاسترجاع) عبر بطاقتك البنكية، ثم يمكنك المزايدة مباشرة.',
    ],

    // ===== Shared field labels & placeholders =====
    'nin_label' => 'رقم التعريف الوطني (NIN)',
    'login_id_label' => 'رقم التعريف الوطني أو البريد الإلكتروني',
    'login_id_placeholder' => 'رقم التعريف أو البريد الإلكتروني',
    'first_name_ar_label' => 'الاسم بالعربية',
    'first_name_placeholder' => 'الاسم',
    'last_name_ar_label' => 'اللقب بالعربية',
    'last_name_placeholder' => 'اللقب',
    'phone_label' => 'رقم الهاتف',
    'email_label' => 'البريد الإلكتروني',
    'birth_date_label' => 'تاريخ الميلاد',
    'password_label' => 'كلمة المرور',
    'new_password_label' => 'كلمة المرور الجديدة',
    'password_confirm_label' => 'تأكيد كلمة المرور',
    'otp_label' => 'رمز التحقق',

    // ===== Login =====
    'login_title' => 'تسجيل الدخول',
    'login_subtitle' => 'أدخل بياناتك للوصول إلى حسابك على منصة مزايدة.',
    'forgot_password' => 'نسيت كلمة المرور؟',
    'login_button' => 'تسجيل الدخول',
    'no_account' => 'ليس لديك حساب؟',
    'create_account_link' => 'أنشئ حساباً جديداً',

    // ===== Register =====
    'register_title' => 'إنشاء حساب',
    // :terms and :privacy are replaced with links from the view.
    'terms_agree' => 'أوافق على :terms و:privacy لمنصة مزايدة.',
    'terms_link' => 'شروط الاستخدام',
    'privacy_link' => 'سياسة الخصوصية',
    'register_button' => 'إنشاء الحساب',
    'have_account' => 'لديك حساب؟',
    'login_link' => 'تسجيل الدخول',

    // ===== Verify OTP =====
    'otp_title' => 'تأكيد رمز التحقق',
    'otp_subtitle' => 'أدخل الرمز المكوّن من 6 أرقام الذي أرسلناه إلى بريدك الإلكتروني.',
    'otp_button' => 'تأكيد الرمز',
    'resend' => 'إعادة الإرسال',
    'resend_hint' => 'لم تستلم الرمز؟ تأكد من بريدك الإلكتروني أو مجلد الرسائل غير المرغوب فيها (Spam).',

    // ===== Reset password =====
    'reset_title' => 'استعادة كلمة المرور',
    'reset_subtitle_request' => 'أدخل رقم التعريف الوطني والبريد الإلكتروني لاستلام رمز التحقق.',
    'reset_subtitle_confirm' => 'أدخل رمز التحقق وكلمة المرور الجديدة.',
    'change_password_button' => 'تغيير كلمة المرور',
    'send_otp_button' => 'إرسال رمز التحقق',
    'back_to_login' => 'العودة لتسجيل الدخول',

    // ===== Controller flash / error messages =====
    'too_many_attempts' => 'عدد المحاولات الفاشلة كبير. حاول مجدداً خلال :sec ثانية.',
    'account_locked' => 'الحساب مقفل. حاول مجدداً بعد :time',
    'invalid_credentials' => 'بيانات الدخول غير صحيحة.',
    'account_blocked' => 'تم حظر هذا الحساب.',
    'otp_invalid' => 'رمز التحقق غير صحيح أو منتهي الصلاحية.',
    'otp_resent' => 'تم إعادة إرسال رمز التحقق إلى بريدك الإلكتروني.',
    'otp_resend_cooldown' => 'الرجاء الانتظار :sec ثانية قبل طلب رمز جديد.',
    'otp_too_many_attempts' => 'عدد المحاولات كبير جداً. الرجاء طلب رمز جديد.',
    'verify_email_first' => 'يجب تأكيد بريدك الإلكتروني أولاً. أرسلنا لك رمزاً جديداً.',
    'account_not_found' => 'لم يتم العثور على الحساب.',
    'otp_sent' => 'تم إرسال رمز التحقق.',
    'reset_otp_sent' => 'إذا كان الحساب موجوداً، فقد أرسلنا رمز التحقق إلى بريدك الإلكتروني.',
    'password_changed' => 'تم تغيير كلمة المرور بنجاح.',
    'password_mismatch' => 'كلمة المرور المدخلة لا تطابق كلمة مرورك الحالية.',

    'logout' => 'تسجيل الخروج',
    'lost_email_recover' => 'فقدت بريدك الإلكتروني؟',
    'recover_title' => 'استرجاع الحساب',
    'recover_subtitle_identify' => 'أدخل رقم التعريف الوطني والبريد الإلكتروني لعرض سؤال الأمان.',
    'recover_subtitle_answer' => 'أجب عن سؤال الأمان لتعيين كلمة مرور جديدة.',
    'recover_your_question' => 'سؤال الأمان الخاص بك',
    'recover_answer_label' => 'إجابتك',
    'recover_continue' => 'متابعة',
    'recover_use_email_instead' => 'استخدم استرجاع البريد الإلكتروني بدلاً من ذلك',
    'recover_not_available' => 'لا يتوفر استرجاع بسؤال الأمان لهذه البيانات. يرجى استخدام استرجاع البريد الإلكتروني أو التواصل مع الدعم.',
    'recover_wrong_answer' => 'الإجابة غير صحيحة.',
    'recover_question_fallback' => 'سؤال الأمان',
    'two_factor_title' => 'تفعيل التحقق بخطوتين',
    'two_factor_intro' => 'يجب على موظفي المنصة تفعيل التحقق بخطوتين قبل الدخول إلى لوحة الإدارة.',
    'two_factor_required' => 'التحقق بخطوتين مطلوب للمتابعة.',
    'two_factor_enabled' => 'تم تفعيل التحقق بخطوتين على حسابك.',
    'two_factor_go_dashboard' => 'الذهاب إلى لوحة التحكم',
    'two_factor_scan' => 'امسح رمز QR بتطبيق المصادقة ثم أدخل الرمز المكوّن من 6 أرقام.',
    'two_factor_code_label' => 'رمز التحقق',
    'two_factor_confirm' => 'تأكيد التفعيل',
    'two_factor_cancel' => 'إلغاء',
    'two_factor_enable_hint' => 'ستحتاج إلى تطبيق مصادقة (مثل Google Authenticator).',
    'two_factor_enable' => 'تفعيل التحقق بخطوتين',

    // ===== Mobile API success messages =====
    'api' => [
        'registered' => 'تم إنشاء الحساب. أرسلنا رمز تحقق إلى بريدك الإلكتروني.',
        'email_verified' => 'تم تأكيد بريدك الإلكتروني بنجاح.',
        'logged_in' => 'تم تسجيل الدخول بنجاح.',
        'logged_out' => 'تم تسجيل الخروج بنجاح.',
        'token_refreshed' => 'تم تجديد الجلسة.',
        'needs_email_verification' => 'يجب تأكيد بريدك الإلكتروني أولاً. أرسلنا لك رمزاً جديداً.',
        'question_revealed' => 'تم العثور على الحساب. أجب عن سؤال الأمان.',
    ],

    'secret_questions' => [
        'mother_maiden' => 'ما هو الاسم العائلي لوالدتك؟',
        'first_school' => 'ما اسم أول مدرسة التحقت بها؟',
        'birth_city' => 'في أي مدينة وُلدت؟',
        'pet_name' => 'ما اسم أول حيوان أليف لديك؟',
        'fav_teacher' => 'من هو معلّمك المفضّل؟',
    ],
];
